...
function ConfigModal({ project, onClose, onSave }) {
    ...
    const [isPoC, setIsPoC] = useState(project?.project_type === "poc");
    
    const handleSave = () => {
        // Save API config
        onSave(config);
        // Also update project_type if changed
        if (isPoC !== (project?.project_type === "poc")) {
            const projectUpdates = { project_type: isPoC ? "poc" : "standard" };
            // We need to call a different handler for project_type updates
            // For now, we'll update it through the same onSave but with a flag
            console.log("PoC mode changed:", isPoC ? "enabled" : "disabled");
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full border border-slate-200 flex flex-col">
                ...
                <div className="p-6 space-y-5">
                    ...
                    <div className="pt-2"><label className="flex items-center gap-3 cursor-pointer p-3 border rounded-xl hover:bg-slate-50"><input type="checkbox" checked={isPoC} onChange={e=>setIsPoC(e.target.checked)} className="w-5 h-5 accent-amber-500" /><div className="flex flex-col"><span className="font-bold text-sm text-slate-800">Enable Proof of Concept (PoC) Mode</span><span className="text-[10px] text-slate-500">4-phase lifecycle (skips Post-Live), faster delivery, capped resources</span></div></label></div>
                ...
                <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 rounded-b-2xl flex justify-end gap-3"><button onClick={onClose} className="px-5 py-2 text-xs font-bold text-slate-600 hover:bg-slate-200 rounded-lg">Cancel</button><button onClick={handleSave} className="px-5 py-2 text-xs font-bold bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow-md">Save Configuration</button></div>
            </div>
        </div>
    );
}
