...
def learn_from_simulation(project_id, trace, summary, resource_usage=None):
    ...
    try:
        ...
        # ── 4. Auto-generate WBS if none exists ──
        has_wbs = bool(pdata.get("migrationPlan") or pdata.get("wbs") or pdata.get("wbsMatrix"))
        if not has_wbs:
            _auto_generate_wbs(project_id, server_patterns, summary, pdata)
            logger.info(f"🧠 Playbook Learner: auto-generated WBS from simulation (project had no pre-migration WBS)")

        # ── 5. Auto-generate runbook from trace ──
...
def _auto_generate_wbs(project_id, server_patterns, summary, pdata):
    """Auto-generate a WBS (migrationPlan) from the simulation patterns.
    Called when a project has no pre-migration WBS but has completed a simulation."""

    phase_labels = {
        "PHASE_4_0": ("Phase 1: Initialization & Triage", "Partner"),
        "PHASE_4_1": ("Phase 2: Network Verification", "Network Engineer"),
        "PHASE_4_2b_PREFLIGHT": ("Phase 3: Source & Agent Preparation", "Customer IT"),
        "PHASE_4_2c_TARGET": ("Phase 4: Target Provisioning", "Migration Engineer"),
        "PHASE_4_2d_SYNC": ("Phase 5: Data Synchronization", "Migration Engineer"),
        "PHASE_4_2f_POST": ("Phase 6: Smoke Tests", "QA Engineer"),
        "PHASE_4_3": ("Phase 7: Landing Zone", "Partner"),
        "PHASE_4_6": ("Phase 8: Cutover", "All Stakeholders"),
        "PHASE_4_7": ("Phase 9: Cleanup", "Migration Engineer"),
        "PHASE_4_8": ("Phase 10: Finalize & Delivery Report", "Partner"),
    }

    # Get all unique phases in order
    all_phases = []
    for sp in server_patterns.values():
        for ph in sp["phases_seen"]:
            if ph not in all_phases:
                all_phases.append(ph)

    # Also include phases from summary if available
    if summary and summary.get("required_phases"):
        for ph in summary["required_phases"]:
            if ph not in all_phases:
                all_phases.append(ph)

    wbs_tasks = []
    for i, ph in enumerate(all_phases, 1):
        label, resp = phase_labels.get(ph, (ph.replace("PHASE_", "Phase "), "Partner"))
        wbs_tasks.append({
            "id": str(i),
            "name": label,
            "isParent": True,
        ...
        # Add sub-tasks based on server patterns
        for sp in server_patterns.values():
            if ph in sp["phases_seen"]:
                # Top 3 actions in this phase for this server
                phase_actions = [a for a in sp["actions"] if a][:3]
                for j, action in enumerate(phase_actions, 1):
                    task_id = f"{i}.{j}"
                    # Avoid duplicates
                    if any(t["id"] == task_id for t in wbs_tasks):
                        continue
                    wbs_tasks.append({
                        "id": task_id,
                        "name": f"{action.replace('_', ' ').title()} — {sp['server_name']}",
                        "isParent": False,
                        "resp": resp,
                        "prog": "0%",
                        "start": "",
    ...
    # Save WBS to project data
    try:
        project = ProjectData.query.get(project_id)
        if project:
            updated = json.loads(project.data) if isinstance(project.data, str) else dict(project.data or {})
            updated["migrationPlan"] = wbs_tasks
            updated["wbsAutoGenerated"] = True
            project.data = json.dumps(updated) if isinstance(project.data, str) else updated
            db.session.commit()
            logger.info(f"🧠 Auto-generated WBS with {len(wbs_tasks)} tasks from simulation trace")
    except Exception as e:
        logger.error(f"Failed to save auto-generated WBS: {e}")
        db.session.rollback()
