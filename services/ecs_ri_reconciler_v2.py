...
class ECSRIReconciler:
    ...
    def __init__(self, raw_ak: str, raw_sk: str, region: str = 'la-south-2'):
        ...
        self.bss_scanner = HuaweiBSSScanner(raw_ak=self.raw_ak, raw_sk=self.raw_sk)
    ...
    def reconcile_ecs_ris(self, quoted_ecs_ris: List[Dict]) -> Dict:
        try:
            ...
            for server in live_ecs_servers:
                ...
                if spec not in live_by_spec: live_by_spec[spec] = []
                live_by_spec[spec].append(server)
            
            bought_by_spec = {}
            for ri in bought_ris:
                spec = ri.get('specification', 'Unknown')
                if spec not in bought_by_spec: bought_by_spec[spec] = []
            ...
            for spec, servers in live_by_spec.items():
                normalized_spec = self._normalize_spec_name(spec)
                if normalized_spec not in normalized_live_by_spec: normalized_live_by_spec[normalized_spec] = []
                normalized_live_by_spec[normalized_spec].extend(servers)
            
            normalized_bought_by_spec = {}
            for spec, servers in bought_by_spec.items():
                normalized_spec = self._normalize_spec_name(spec)
                if normalized_spec not in normalized_bought_by_spec: normalized_bought_by_spec[normalized_spec] = []
            ...
            return {
                'summary': { 'total_quoted': total_quoted, 'total_live': total_live, 'total_bought': total_bought, 'total_missing': total_missing, 'by_specification': by_specification },
