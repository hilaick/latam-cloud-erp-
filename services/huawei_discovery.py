...
class HuaweiDiscovery:
    def __init__(self, encrypted_ak_data: Any, encrypted_sk_data: Any, region: str, master_password: str):
        ...
        try:
            ...
            for p in iam_client.keystone_list_projects(KeystoneListProjectsRequest()).projects or []:
                self.project_ids[p.name] = p.id
        except Exception as e:
            logger.warning(f"IAM auto-resolve failed: {e}")
            ...
            for target_region in self.regions:
                ...
                try:
                    ...
                    for s in ecs_client.list_servers_details(ListServersDetailsRequest(limit=100)).servers or []:
                        inventory["compute"].append({ "id": s.id, "name": s.name, "type": "ECS", "status": s.status, "flavor": s.flavor.name, "vcpus": int(s.flavor.vcpus), "ram_gb": int(s.flavor.ram)/1024, "os_type": s.metadata.get('os_type', 'Unknown'), "private_ip_address": s.addresses.values()[0][0].addr if s.addresses else 'N/A', "region": target_region })
                ...
                try:
                    ...
                    for db in rds_client.list_instances(ListInstancesRequest()).instances or []:
                        inventory["databases"].append({ "id": db.id, "name": db.name, "type": "RDS", "engine": db.datastore.type, "version": db.datastore.version, "status": db.status, "private_ip_address": db.private_ips[0] if db.private_ips else 'N/A', "volume_gb": db.volume.size, "region": target_region })
                ...
                try:
                    ...
                    for sub in vpc_client.list_subnets(ListSubnetsRequest(limit=100)).subnets or []: inventory["network"].append({"id": sub.id, "name": sub.name, "type": "Subnet", "cidr": sub.cidr, "vpc_id": sub.vpc_id, "status": sub.status, "region": target_region})
                ...
                if HAS_VPN:
                    try:
                        ...
                        for cgw in vpn_client.list_customer_gateways(ListCustomerGatewaysRequest(limit=100)).customer_gateways or []: inventory["network"].append({"id": cgw.id, "name": cgw.name, "type": "Customer Gateway", "cidr": cgw.bgp_asn or 'N/A', "status": "Active", "region": target_region})
                    ...
                    except Exception as e: logger.error(f"VPN error: {e}")

                # 5. STORAGE & BACKUP
                if HAS_CBR:
                    try:
                        cbr_client = CbrClient.new_builder().with_credentials(region_creds).with_region(CbrRegion.value_of(target_region)).build()
                        for vault in cbr_client.list_vaults(ListVaultsRequest(limit=100)).vaults or []: inventory["storage"].append({"id": vault.id, "name": vault.name, "type": "CBR", "location": target_region, "status": "Active", "size": getattr(vault, 'size', 0), "used": getattr(vault, 'used', 0)})
