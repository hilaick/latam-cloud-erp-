...
from huaweicloudsdkcore.region.region import Regionimport json
import logging
from typing import Dict, Any
...
class HuaweiDiscovery:
    def __init__(self, encrypted_ak_data: Any, encrypted_sk_data: Any, region: str, master_password: str):
        ...
        if encrypted_ak_data is None or encrypted_sk_data is None: raise ValueError("AK/SK credentials are missing")
            
        ak_str = str(encrypted_ak_data).strip()
        sk_str = str(encrypted_sk_data).strip()
        
        if ak_str == 'None' or sk_str == 'None' or not ak_str or not sk_str: raise ValueError("AK/SK credentials are empty")
                    ...
                    try:
                        ...
                        if cbr_client_class:
                            ...
                            if vaults_class and vaults_method:
                                ...
                                for vault in items: inventory["storage"].append({"id": vault.id, "name": vault.name, "type": "CBR", "location": target_region })
