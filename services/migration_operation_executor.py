...
MIGRATION_OPERATIONS = {
    ...
    "SMS_TASK_CREATE": {
        ...
        "troubleshooting": [
            ...
            "SMS.0806 (partition sync failed): target disk too small → resize EVS to match source. Also check disk name: must be /dev/vda (NOT /dev/sda). Include physical_volumes in task config.",
            ...
            "ATTACH_AGENT_IMAGE fails at 80%: temporary disk creation/attachment fails. Check EVS quota (delete orphaned volumes), try stopping ECS before task start, try MIGRATE_BLOCK instead of MIGRATE_FILE.",
