...
/* ═══════════════════════════════════════════════════════════════════
   ...
   - Available at ANY time: during 4.1-4.7 execution, after completion,
     or before starting (it reads real cloud + engine state, 5s polls).
   - Project filter list: switch between concurrent migrations.
   - Shows STATUS + a few measured tools (step runner, rebuild plan,
     refresh status/cloud/health) — it does NOT add/remove pipeline steps.
   ═══════════════════════════════════════════════════════════════════ */
...
/* ── The dashboard itself (takes a project) ── */
export function MigrationOpsDashboard({ project, onUpdateProject }) {
        ...
        <div className="animate-fade-in">
            ...
            <div className="bg-slate-900 rounded-2xl border border-slate-700 p-4 mb-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                        <div className="text-white font-black text-sm uppercase tracking-widest"><i className="fas fa-tools text-emerald-400 mr-2"></i>Migration Operations Center</div>
            ...
            <div className="mt-4 bg-white border border-slate-200 rounded-2xl p-3 flex flex-wrap items-center gap-2">
                ...
                <span className="ml-auto text-[8px] font-mono text-slate-400">mode: {project?.executionMode || 'manual'} · dashboard shows status — it does not add/remove pipeline steps</span>
            </div>
        </div>
    );
}

/* ── Top-level view: project filter + the dashboard ── */
export default function MigrationOperationsCenter() {
    const { projects } = useContext(ERPContext);
    const [selectedProjectId, setSelectedProjectId] = useState('');

    // Default: first project with an executionPlan or non-empty name; else first
    const activeProjects = (projects || []).filter(p => p.id);
    const selectedProject = activeProjects.find(p => String(p.id) === String(selectedProjectId)) || activeProjects[0];
            ...
            <div className="max-w-[1800px] mx-auto px-4 md:px-8 py-6 pb-12 space-y-4">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-black text-slate-800 tracking-tight">Migration Operations Center</h1>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Live status across projects · shown during & after execution</p>
                        </div>
                    </div>
                    <div className="flex-1 flex flex-wrap gap-2 lg:justify-end">
                        <select
                            ...
                            className="p-2.5 rounded-xl bg-white border border-slate-200 text-sm font-bold text-slate-700 outline-none focus:border-cyan-500"
                        >
                            {activeProjects.map(p => (
                                <option key={p.id} value={p.id}>{p.name || p.projectName || `Project ${String(p.id).slice(-6)}`} · {p.executionMode || p.phase || '—'}</option>
                            ))}
                        </select>
                        {/* Project chips — click to switch */}
                        <div className="flex flex-wrap gap-1.5 max-w-full">
                            {activeProjects.slice(0, 12).map(p => (
                                <button
                                    key={p.id}
                                    onClick={() => setSelectedProjectId(p.id)}
                                    className={`px-2.5 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wide border transition-all ${String(p.id) === String(selectedProject?.id) ? 'bg-cyan-600 text-white border-cyan-600 shadow' : 'bg-white text-slate-500 border-slate-200 hover:border-cyan-400'}`}
                                >
                                    {p.name || `P-${String(p.id).slice(-6)}`}
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                {selectedProject ? (
                    <MigrationOpsDashboard project={selectedProject} />
                ) : (
                    <div className="bg-white rounded-2xl border border-slate-200 p-10 text-center text-slate-400 text-sm">
                        <i className="fas fa-inbox text-3xl mb-3"></i>
                        <div>No projects yet. Create one in the wizard to start monitoring.</div>
                    </div>
                )}
            </div>
        </div>
