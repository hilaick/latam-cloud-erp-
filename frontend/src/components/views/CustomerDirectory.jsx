...
export default function CustomerDirectory() {
    ...
    const initiateDeleteCustomer = (customerId) => {
        ...
        // Block if projects are attached (prevents orphaned projects)
        ...
        if (attachedProjects.length > 0) {
            alert(`Access Denied: Cannot delete customer "${customer.name}". They have ${attachedProjects.length} active project(s) attached. Please reassign or delete the projects first.`);
            return; 
        }
    ...
    // 🚨 Execution Phase (Post-2FA)
    const executeDelete = () => {
        if (!customerToDelete) return;
        // handleDeleteCustomer from context manages the API and safe state update
                        ...
                        <div className="relative z-10">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button onClick={() => initiateDeleteCustomer(c.id)} className="text-slate-400 hover:text-rose-600 transition-colors"><i className="fas fa-trash"></i></button>
                                </div>
                            </div>
