                    ...
                    <div className="flex-1 overflow-y-auto p-2 space-y-1">
                        ...
                        {customers.map(c => (
                            <div 
                                ...
                                className={`p-4 rounded-xl cursor-pointer transition-colors border-2 ${selectedId === c.id ? 'border-blue-500 bg-blue-50 shadow-sm' : 'border-transparent hover:bg-slate-50'}`}
                            >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-4">
                                    <h4 className="text-xs font-black uppercase tracking-widest text-slate-500 mb-4">
                                        <i className="fas fa-lock text-emerald-500 mr-2"></i> Security Vault (API Credentials)
                                    </h4>
                                    <div>
                                ...
                                <div>
                                    <h4 className="text-xs font-black uppercase tracking-widest text-slate-500 mb-4">
                                        <i className="fas fa-folder-open text-blue-500 mr-2"></i> Active Portfolio
                                    </h4>
                                            ...
                                            <div key={p.id} className="p-4 bg-slate-50 border border-slate-200 rounded-xl flex justify-between items-center">
                                                <div>
                                                    ...
                                                    <div className="text-[10px] text-slate-500 mt-1 uppercase font-bold">
                                                        <i className="fas fa-globe-americas mr-1"></i> {p.country || 'Global'} | SA: {p.sa}
                                                    </div>
                                                </div>
                                                <div className="text-xs font-black text-emerald-600 bg-emerald-50 px-2 py-1 rounded border border-emerald-200">
                                                    ${p.mrr}
                                                </div>
                                            </div>
                                        ))}
