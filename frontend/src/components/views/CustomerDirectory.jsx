...
export default function CustomerDirectory() {
    ...
    const handleSave = () => {
        ...
        else handleUpdateCustomer(editingCustomer); // Uses the updated API endpoint
                ...
                <div className="fixed inset-0 z-[100] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col border border-slate-700 animate-slide-up max-h-[90vh]">
                        <div className="bg-slate-900 px-8 py-5 rounded-t-2xl flex justify-between items-center text-white shrink-0">
                            <div><h3 className="font-black text-xl text-blue-400"><i className="fas fa-user-shield mr-3"></i> Customer Profile & Vault</h3></div>
                        ...
                        <div className="flex border-b border-slate-200 bg-slate-50 shrink-0 overflow-x-auto">
                            ...
                            {/* 🚨 THE MISSING MULTI-CLOUD TAB RESTORED */}
                            ...
                            {activeTab === 'general' && (
                                <div className="space-y-5 animate-fade-in">
                                    <div className="grid grid-cols-2 gap-5">
                                        ...
                                        <div><label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Default Cloud Region</label><input type="text" value={editingCustomer.region || ''} onChange={e=>setEditingCustomer({...editingCustomer, region: e.target.value})} className="w-full p-3 border border-slate-300 rounded-lg text-sm font-mono outline-none focus:border-blue-500" /></div>
                                    </div>
                                </div>
                            )}
