...
export default function UserManagement() {
    ...
    return (
        <div className="animate-fade-in max-w-[1600px] mx-auto space-y-6 pb-12">
            <div className="bg-slate-900 rounded-2xl shadow-xl p-8 flex flex-col md:flex-row justify-between items-center text-white border border-slate-700 gap-4">
                <div>
                    <h2 className="text-3xl font-black mb-2"><i className="fas fa-users-cog text-blue-400 mr-3"></i> IAM & User Management</h2>
                    <p className="text-sm text-slate-400">Role-Based Access Control (RBAC) and Security Governance.</p>
                </div>
                <button className="px-6 py-3 bg-blue-600 hover:bg-blue-700 rounded-xl text-xs font-black uppercase tracking-widest shadow-md transition-transform active:scale-95 whitespace-nowrap"><i className="fas fa-user-plus mr-2"></i> Invite User</button>
            </div>
                        ...
                        <tbody className="divide-y divide-slate-100 text-sm">
                            {users.map(u => (
                                <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="p-4">
                                        {u.is2fa ? <span className="bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full text-[10px] font-black border border-emerald-200"><i className="fas fa-shield-check mr-1"></i> Enabled</span> : <span className="bg-rose-50 text-rose-700 px-3 py-1 rounded-full text-[10px] font-black border border-rose-200"><i className="fas fa-exclamation-triangle mr-1"></i> Disabled</span>}
                                    </td>
                                    <td className="p-4">
                                        <div className={`text-xs font-black ${u.status==='Active'?'text-emerald-600':'text-amber-600'}`}>{u.status}</div>
                                        <div className="text-[10px] text-slate-400 mt-1 font-bold">{u.lastLogin}</div>
                                    </td>
                                    <td className="p-4 text-center">
                                        <button disabled={u.role==='Master Admin'} className="p-2 text-slate-400 hover:text-rose-600 transition-colors disabled:opacity-30"><i className="fas fa-trash-alt"></i></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
