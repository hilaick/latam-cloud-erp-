...
export default function GlobalSchedule() {
    ...
    const timelineProjects = useMemo(() => {
        ...
        (projects||[]).filter(p => p && !p.isWaiting).forEach(p => { 
            ...
            // 🚨 FIXED: Pulling from multiple possible CRM date fields
            ...
            if(!isNaN(start.getTime()) && !isNaN(end.getTime()) && end >= start) {
                ...
                valid.push({ ...p, startObj: start, endObj: end, targetNodes, kickoffStr, targetStr }); 
            }
        });
                ...
                {timelineProjects.length === 0 ? (
                                ...
                                {timelineProjects.map(p => (
                                    <div key={p.id} className="flex items-center group">
                                        ...
                                        <div className="flex-1 h-12 relative bg-slate-50 border-y border-r border-transparent group-hover:bg-slate-100 transition-colors rounded-r-lg ml-2">
                                            ...
                                            <div onClick={() => navigateToProject(p.id)} className={`absolute top-1 bottom-1 rounded-md shadow-md border-2 flex flex-col justify-center px-3 overflow-hidden transition-transform hover:scale-[1.02] cursor-pointer ${p.health === 'Green' ? 'bg-emerald-500 border-emerald-600 text-white' : p.health === 'Red' ? 'bg-rose-500 border-rose-600 text-white' : 'bg-amber-400 border-amber-500 text-slate-900'}`} style={{ left: `${getLeftPos(p.startObj)}%`, width: `${Math.max(getWidth(p.startObj, p.endObj), 2)}%`, minWidth:'70px'}} title={`Status: ${p.lifecycleState}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 animate-fade-in">
                        {timelineProjects.sort((a,b) => a.endObj - b.endObj).map(p => {
                            ...
                            return (
                                <div key={p.id} onClick={() => navigateToProject(p.id)} className={`relative p-6 rounded-2xl border-2 cursor-pointer transition-all hover:-translate-y-1 shadow-sm hover:shadow-lg ${isThisWeek ? 'bg-rose-50 border-rose-200' : isPast ? 'bg-slate-50 border-slate-200 opacity-70' : 'bg-white border-slate-200 hover:border-emerald-300'}`}>
                                    {isThisWeek && <div className="absolute -top-3 -right-3 bg-rose-500 text-white text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full shadow-md animate-pulse">Critical Weekend</div>}
                                    {isPast && <div className="absolute top-4 right-4 text-[10px] font-black uppercase tracking-widest text-slate-400 border border-slate-300 px-2 py-0.5 rounded bg-white">Past</div>}
                                    
                                    <div className="flex items-center gap-3 mb-4">
                                        <div className={`w-12 h-12 rounded-xl flex flex-col items-center justify-center shadow-inner ${isThisWeek ? 'bg-rose-500 text-white' : isPast ? 'bg-slate-200 text-slate-500' : 'bg-emerald-100 text-emerald-700'}`}>
                                        ...
                                        <div>
                                            <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Go-Live Cutover</div>
                                            <h4 className="font-black text-slate-800 text-sm truncate w-48">{p.name}</h4>
                                        </div>
                                    </div>
                                    
                                    <div className="flex gap-2 mb-4">
                                        <span className="bg-slate-100 border border-slate-200 text-slate-600 px-2 py-1 rounded text-[9px] font-black uppercase tracking-widest"><i className="fas fa-user-tie mr-1"></i> {p.sa || 'TBD'}</span>
                                        <span className="bg-indigo-50 border border-indigo-100 text-indigo-700 px-2 py-1 rounded text-[9px] font-black uppercase tracking-widest"><i className="fas fa-server mr-1"></i> {p.targetNodes} Nodes</span>
                                    </div>
                                    
                                    <div className="bg-slate-100 h-2 rounded-full overflow-hidden shadow-inner">
                                        <div className={`h-full ${p.health === 'Red' ? 'bg-rose-500' : 'bg-emerald-500'}`} style={{width: p.progress || '0%'}}></div>
                                    </div>
                                    <div className="text-right text-[10px] font-black text-slate-500 mt-1">{p.progress || '0%'} Readiness</div>
                                </div>
                            );
                        })}
                    </div>
