...
export default function PhysicsEngine({ project, onUpdateProject }) {
    // 1. Global Network Constraints (Applies to all waves)
    ...
    // 2. Load Waves from Topology Mapper
    ...
    const waves = useMemo(() => {
        ...
        computeAndDb.forEach(n => {
            ...
            if (!groups[waveName]) groups[waveName] = { name: waveName, nodes: [], nodeCount: 0, isDbHeavy: false };
            groups[waveName].nodes.push(n);
            groups[waveName].nodeCount++;
            if (['RDS', 'GAUSSDB', 'DB'].includes(String(n.type).toUpperCase())) groups[waveName].isDbHeavy = true;
        });
        
        return Object.values(groups);
    }, [nodes]);

    // 3. Wave-Specific Configurations
    // We store the storage size and CPU config per wave in a dictionary
    ...
    useEffect(() => {
        if (project?.physics?.waveConfigs) {
            ...
            // Initialize default configs for newly detected waves
            const defaults = {};
            waves.forEach(w => {
                defaults[w.name] = { storageSizeTB: (w.nodeCount * 0.5).toFixed(1), computeCPU: 60 };
            });
    ...
    const handleWaveConfigChange = (waveName, field, value) => {
        setWaveConfigs(prev => ({
            ...prev,
            [waveName]: { ...prev[waveName], [field]: value }
        }));
    };

    const saveContext = () => { 
        const data = { waveConfigs, netSource, transitType, netTunnel, targetKMS, calculatedTotalHours: longestWaveHours };
        onUpdateProject(project.id, 'physics', data); 
        alert("Wave Physics Parameters Saved."); 
    };

    // 4. THE WAVE MATH ENGINE
    const waveResults = useMemo(() => {
        return waves.map(wave => {
            const config = waveConfigs[wave.name] || { storageSizeTB: 1.0, computeCPU: 60 };
            
            // Step A: Calculate Pipeline Speed
            ...
            // Add DB Logical overhead if wave is DB heavy
            const finalHours = wave.isDbHeavy ? syncHours * 1.5 : syncHours;

            return {
                ...wave,
                payloadTB,
                actualMbps: actualTransferMbps.toFixed(0),
    ...
    if (waves.length === 0) {
        return (
            <div className="bg-white rounded-2xl border-2 border-dashed border-slate-300 p-12 text-center text-slate-500 animate-fade-in">
                ...
                <p className="font-medium text-sm">Please return to Step 2 (Architecture) and populate the Topology Mapper. The Physics Engine calculates SLAs based on the application groups defined in the topology.</p>
            </div>
        );
    }
        ...
        <div className="max-w-[1600px] mx-auto space-y-6 pb-12 animate-fade-in">
            ...
            <div className="bg-slate-900 rounded-2xl shadow-xl p-8 flex justify-between items-center text-white border border-slate-700">
                ...
                <div className="flex gap-6 items-center text-right">
                    <div><div className="text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-1">Total Payload</div><div className="text-2xl font-black text-purple-400">{totalPayload} TB</div></div>
                    <div className="border-l border-slate-700 pl-6"><div className="text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-1">Longest Wave SLA</div><div className="text-3xl font-black text-emerald-400">{longestWaveHours}h</div></div>
                    <button onClick={saveContext} className="ml-4 px-6 py-3 bg-blue-600 hover:bg-blue-700 rounded-xl text-xs font-black uppercase tracking-widest shadow-md transition-transform active:scale-95">Save Wave Math</button>
                </div>
            </div>
                        ...
                        return (
                            <div key={wave.name} className="bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row overflow-hidden hover:border-blue-300 transition-colors">
                                ...
                                <div className="bg-slate-50 border-r border-slate-200 p-6 md:w-64 flex flex-col justify-center relative overflow-hidden">
                                    ...
                                    <p className="text-[10px] uppercase tracking-widest text-slate-500 font-bold mt-1 relative z-10">{wave.nodeCount} Servers {wave.isDbHeavy && ' + Database'}</p>
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            <label className="flex justify-between text-[10px] font-black tracking-widest uppercase mb-2 text-slate-500">
                                                <span>Estimated Total Storage (TB)</span>
                                            </label>
                                            <input 
